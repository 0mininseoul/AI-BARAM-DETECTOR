...
import { getSelfHostedProfileGlobalGateConfig } from '@/lib/services/instagram/providers/selfhosted/global-request-gate';
...
export function maximumSelfHostedProfileRuntimeMs(
    ...
    const globalGateConfig = getSelfHostedProfileGlobalGateConfig(env);
    ...
    const coordinationOverheadMs = Math.max(
        globalGateConfig.responseGuardMs,
        globalGateConfig.rpcTimeoutMs
    );
    const admissionGateAttemptMs = globalGateConfig.enabled
        ? globalGateConfig.admissionMaxWaitMs + coordinationOverheadMs
        : 0;
    return attempts * (
        config.timeoutMs
        + config.minIntervalMs
        + admissionGateAttemptMs
    ) + retryWaitMs;
}
