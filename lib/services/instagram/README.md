...
The pre-payment target preflight and checkout-time fresh admission are selfhosted-first. Only a classified selfhosted provider failure may enter one Apify profile-summary fallback per preflight, with `maxTotalChargeUsd=$0.0026`; an explicit selfhosted not-found result does not fallback. Both stages bind the same PII-free `analysis_preflight_provider_runs` row. The ledger is reserved before Actor start, retries and fresh admission resume the same stored run ID instead of starting another run, and authenticated usage is reconciled no earlier than 30 seconds after the run settles. If fresh admission rereads a successful run created by the initial preflight, its counts are the preflight-time provider snapshot rather than a second paid snapshot. The selected plan still fails closed when those declared counts exceed its capacity, and relationship collection independently requires at least 99% of each declared side, so a lower plan cannot silently produce a partial result. An expired or abandoned preflight retains its run ledger until reconciliation. Neither path supplies an Instagram login cookie or session.
